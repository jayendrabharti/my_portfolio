...
export default function ContactPage() {
    ...
    const handleSubmit = async (values, { resetForm }) => {
        ...
        const res = JSON.parse(await CreateMessage(values));
        if (res.success) {
            resetForm();
            setSent(true);
        } else {
            alert("Failed to send message! Refesh and Retry!!.");
            resetForm();
    ...
    if(sent)return(
        <div className="w-full h-full flex items-center justify-center flex-col text-green-500 gap-2">
            ...
            >Message was sent successfully!!</span>

            <Link href={'/'} className="bg-slate-800 no-underline group cursor-pointer relative shadow-2xl shadow-zinc-900 rounded-full p-px text-xs font-semibold leading-6  text-black dark:text-white inline-block active:scale-75 transition-all duration-200">
        ...
        <section>
            <div className="py-8 lg:py-16 px-4 mx-auto max-w-screen-md">
                ...
                <p className="mb-8 lg:mb-16 font-light text-center text-zinc-600 dark:text-zinc-400 sm:text-xl">
                    Want a website/webapp made? Contact me and i will respond as soon as i can.
                </p>
                    ...
                    {({ isSubmitting }) => (
                        <Form className="space-y-8">
                            ...
                            <button
                                ...
                                className="p-3 font-bold text-center text-white rounded-lg bg-blue-500 hover:bg-zinc-700 dark:hover:bg-zinc-600 focus:ring-4 focus:outline-none focus:ring-zinc-300 dark:focus:ring-zinc-500 text-md transition duration-300 group flex flex-row"
                            >
